...
class GGNN_Baseline(nn.Module):
    def __init__(self, ggnn, classifier, encoder):
        super(GGNN_Baseline, self).__init__()
        self.ggnn = ggnn
        self.classifier = classifier
        self.encoder = encoder

    def forward(self, image_embeddings, verb_embeddings, role_embeddings, verbs):
        img_features = image_embeddings
        ...
        verb_embd = verb_embeddings
        role_embd = role_embeddings
        ...
        role_label_pred = logits.contiguous().view(img_features.size(0), self.encoder.max_role_count, -1)
...
def build_ggnn_baseline(n_roles, n_verbs, num_ans_classes, encoder):
    ...
    return GGNN_Baseline(ggnn, classifier, encoder)