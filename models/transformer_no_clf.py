...
class TransformerModel(nn.Module):
    def __init__(self, num_classes, encoder, num_layers, num_heads):
        ...
        noun_dict = pickle.load(open('data/processed/clip_noun_embeds.pkl','rb'))
        self.uniq_val_emb = torch.stack(list(noun_dict.values())).float().cuda()
        self.w = nn.Parameter(torch.randn(512,128))
        self.w2 = nn.Parameter(torch.randn(512,512))
        # Define the transformer layer
        transformer_encoder_layer = nn.TransformerEncoderLayer(d_model=512, nhead=num_heads, dim_feedforward=2048, dropout=0.1, activation='relu')
    ...
    def forward(self, image_emb, verb_emb, role_emb,mask):
        ...
        q_proj = output.view(-1, output.shape[-1])
        cor_q = (q_proj@self.w @ (q_proj@self.w).T)
        det = torch.det(cor_q)
        cor_q = cor_q @ (q_proj @ self.w2)
        # normalize
        cor_q = cor_q / det
        q_proj = q_proj + cor_q

        # uniq_val_emb - self.num_ans_classes x 512
        # # similarity - bs*num_roles x self.num_ans_classes
        #sim = torch.einsum('np,pb->nb', self.uniq_val_emb, q_proj.transpose(1,0)).transpose(1,0)
        #logits = sim.view(-1, self.encoder.max_role_count, self.num_ans_classes)
        q_proj = q_proj.view(-1, self.encoder.max_role_count, 512)
        logits = self.classifier(q_proj)
...
def build_transformer_no_clf(num_ans_classes, encoder, num_layers, num_heads):
