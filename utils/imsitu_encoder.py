...
class imsitu_encoder():
    def __init__(self, train_set, dev_set=None, test_set=None):
        ...
        self.bb_classdict = {}
        with open('SWiG_jsons/train_classes.csv') as f:
            csv_reader = csv.reader(f, delimiter=',')
            for row in csv_reader:
                self.bb_classdict[row[0]] = row[1]
        ...
        combined = dict(list(dev_set.items()) + list(test_set.items()))
        for img_id in combined:
            img = combined[img_id]
            current_verb = img['verb']
            for frame in img['frames']:
                for role,label in frame.items():
    ...
    def get_bb_mask_loc(self, verb, bb_dict, width, height):
        ...
        for role in roles:
            bb = bb_dict[role]
            bb_loc.append(torch.tensor(bb))
            if bb == [-1, -1, -1, -1]:
                bb_mask.append(0) 
            else:
                bb_mask.append(1)
        bb_loc = box_ops.swig_box_xyxy_to_cxcywh(torch.stack(bb_loc), width, height)        
        return bb_mask, bb_loc

    def encode(self, item):
        verb = self.verb_list.index(item['verb'])
        labels = self.get_label_ids(item['verb'], item['frames'])
        try:
            bb_mask, bb_loc = self.get_bb_mask_loc(item['verb'], item['bb'], item['width'], item['height'])
        except:
            bb_mask = None
            bb_loc = None
            width = None
            height = None
        return verb, labels, bb_mask, bb_loc
